"use client";
import React, { useState } from "react";
import Link from "next/link";
import Image from "next/image";
import info from "@/public/Assets/data/specialdata";

function Special() {
  const [isHovered, changeHover] = useState([true, false, false, false]);
  const [data, setData] = useState({
    img: info[0].img,
    info: info[0].info,
  });

  function handleHover(event) {
    const idx = parseInt(event.target.id, 10);
    setTimeout(() => {
      changeHover(() => {
        const arr = [false, false, false, false];
        arr[idx] = true;
        return arr;
      });
      const item = info.filter((infoItem, index) => index === idx)[0];
      setData(() => ({
        img: item.img,
        info: item.info,
      }));
    }, 15);
  }

  return (
    <section className="py-16 px-4 sm:px-8 md:px-20 bg-green-50 rounded-3xl sm:rounded-[70px]">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row lg:space-x-36 space-y-12 lg:space-y-0">
          {/* Links Section */}
          <div className="w-full lg:w-3/5 flex flex-col my-auto mx-auto space-y-6 text-center lg:text-left">
            <h5 className="text-base sm:text-xl font-serif font-semibold mb-6 text-teal-700 tracking-wide uppercase">
              Features Specialities
            </h5>

            <nav className="flex flex-col space-y-5">
              {info.map((item, i) => (
                <Link
                  href={item.title === "Know More about Diseases" ? "/Search" : "#"}
                  key={i}
                  id={i.toString()}
                  onMouseOver={handleHover}
                  onFocus={handleHover} // accessibility
                  className={`font-serif text-2xl sm:text-3xl lg:text-4xl font-extrabold cursor-pointer transition-transform duration-200 ${
                    isHovered[i]
                      ? "underline decoration-teal-600 decoration-4 scale-105 text-teal-800"
                      : "text-teal-600 hover:underline hover:decoration-2 hover:decoration-teal-500"
                  } focus:outline-none focus:ring-2 focus:ring-teal-400 rounded`}
                >
                  {item.title}
                </Link>
              ))}
            </nav>
          </div>

          {/* Image and Info Section */}
          <div className="w-full lg:w-2/5 flex flex-col space-y-6 items-center lg:items-start px-0 sm:px-6 lg:px-0 pr-5">
            <div className="w-72 sm:w-80 md:w-[325px] rounded-2xl overflow-hidden shadow-lg shadow-gray-800/40 transition-transform duration-500 ease-in-out">
              <Image
                src={data.img}
                alt="Consultation"
                width={325}
                height={400}
                className="rounded-2xl object-cover transform transition-transform duration-500 ease-in-out hover:scale-[1.06]"
                priority
              />
            </div>
            <p className="max-w-[340px] text-gray-900 font-medium font-sans text-center lg:text-left text-sm sm:text-base leading-relaxed px-3 sm:px-0">
              {data.info}
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}

export default Special;
